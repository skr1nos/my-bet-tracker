...
div[data-testid="stElementContainer"]:has(.marker-gold),
div[data-testid="stElementContainer"]:has(.marker-dark) {
...
div[data-testid="stElementContainer"]:has(.fab-marker) + div[data-testid="stElementContainer"] {
    position: fixed !important; bottom: 30px !important; right: 30px !important; z-index: 999999 !important; width: auto !important;
}
div[data-testid="stElementContainer"]:has(.fab-marker) + div[data-testid="stElementContainer"] button {
    ...
    border: none !important; box-shadow: 0 10px 25px rgba(16, 185, 129, 0.4) !important;
}
div[data-testid="stElementContainer"]:has(.fab-marker) + div[data-testid="stElementContainer"] button p {
...
div[role="radiogroup"] > label {
    background-color: #16263b !important; padding: 12px 15px !important; 
    border-radius: 8px !important; border: 1px solid #1e3a5f !important; margin-bottom: 12px !important; 
}
</style>
"""
...
# ==========================================
# 📊 CALLBACK FUNCTIONS (LIVE MULTIPLIER)
# ==========================================
...
def render_ticket_html(aa_val, df_source):
    ...
    if stamp_text:
        html_parts.append(f"<div style='position:absolute; top:50px; right:10px; color:{stamp_color}; font-size:65px; font-weight:900; transform:rotate(-15deg); border:4px solid {stamp_color}; padding:5px 15px; border-radius:15px; z-index:0; pointer-events:none; letter-spacing: 2px;'>{stamp_text}</div>")
    ...
    <div style="display: flex; justify-content: space-between; margin-bottom: 20px; position: relative; z-index: 1;">
        <div>
            <p style="margin: 0; font-size: 11px; color: #4db8ff; text-transform: uppercase; letter-spacing: 1px;">ΑΘΛΗΜΑ</p>
            <p style="margin: 0; font-size: 17px; font-weight: 600; color: #e2e8f0;">{row['Sport']}</p>
        </div>
        <div style="text-align: right;">
            <p style="margin: 0; font-size: 11px; color: #4db8ff; text-transform: uppercase; letter-spacing: 1px;">ΤΥΠΟΣ</p>
    ...
    <div style="border-top: 2px dashed #2a4365; padding-top: 20px; position: relative; z-index: 1;">
        <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 15px;">
            <div>
                <p style="margin: 0; font-size: 12px; color: #718096; text-transform: uppercase;">ΠΟΝΤΑΡΙΣΜΑ</p>
                <p style="margin: 0; font-size: 18px; font-weight: 600; color: #e2e8f0;">{row['Stake']:.2f} €</p>
            </div>
            <div style="text-align: right;">
                <p style="margin: 0; font-size: 12px; color: #718096; text-transform: uppercase;">ΣΥΝ. ΑΠΟΔΟΣΗ</p>
        ...
        <div style="background-color: rgba(0,0,0,0.2); padding: 15px; border-radius: 12px; display: flex; justify-content: space-between; align-items: center;">
            <div>
                <p style="margin: 0; font-size: 12px; color: #a8dadc; text-transform: uppercase;">ΣΥΝ. ΕΠΙΣΤΡΟΦΗ</p>
                <p style="margin: 0; font-size: 24px; font-weight: 700; color: #ffffff;">{total_return:.2f} €</p>
            </div>
            <div style="text-align: right;">
                <p style="margin: 0; font-size: 12px; color: #a8dadc; text-transform: uppercase;">ΚΑΘΑΡΟ ΚΕΡΔΟΣ</p>
...
def show_progression_dialog(metric_type, prog_dataframe, full_df):
    ...
    if metric_type == "profit":
        ...
        cfg = {"Τρέχον Ταμείο": st.column_config.NumberColumn("Τρέχον Ταμείο (€)", format="%.2f €"), "Κέρδος/Ζημιά": st.column_config.NumberColumn("Κέρδος/Ζημιά", format="%.2f €"), "Ποντάρισμα": st.column_config.NumberColumn("Ποντάρισμα", format="%.2f €"), "Ημ/νια": st.column_config.DateColumn("Ημ/νια", format="DD/MM/YYYY")}
...
def new_bet_dialog():
    ...
    if bet_type == "Μονό":
        ...
        for i in range(int(num_legs)):
            ...
            l_ev_key = f"pbb_ev_{i}_{reset_id}"
            l_ev = c_ev.text_input(f"Αγώνας {i+1}:", key=l_ev_key)
            render_suggestions(c_ev, l_ev_key, l_ev, get_event_suggestions, (all_events, all_teams))
            
            l_od = c_od.number_input(f"Απόδοση Αγώνα:", min_value=1.00, step=0.01, value=1.50, key=f"leg_od_{i}_{reset_id}", on_change=update_auto_odds, args=(reset_id, num_legs))
            ...
            l_ma = st.text_area(f"Επιλογές Bet Builder (Μία ανά γραμμή):", height=80, key=l_ma_key, placeholder="π.χ.\n1 & Over 2.5\nΣκοράρει ο Vini")
        ...
        for i in range(int(num_legs)):
            ...
            l_od = cc3.number_input(f"Απόδοση {i+1}", min_value=1.00, step=0.01, value=1.50, key=f"leg_od_{i}_{reset_id}", on_change=update_auto_odds, args=(reset_id, num_legs))
            l_st = cc4.selectbox(f"Κατάστ. {i+1}", STATUS_LIST, key=f"leg_st_{i}_{reset_id}", on_change=update_auto_odds, args=(reset_id, num_legs))
    ...
    if filtered_df.empty:
        ...
        for idx, row in completed_bets.sort_values(by="DateTime").iterrows():
            ...
            if status == "🟢 Κερδισμένο":
                ...
                if current_w > max_win_streak: 
                    max_win_streak = current_w; win_streak_idx = curr_w_idx.copy()
            elif status == "🔴 Χαμένο":
                current_l += 1; curr_l_idx.append(row['Α/Α'])
                current_w = 0; curr_w_idx = []
                if current_l > max_lose_streak: 
                    max_lose_streak = current_l; lose_streak_idx = curr_l_idx.copy()
            else: 
        ...
        for idx, row in daily_profits.iterrows():
            ...
            if p > 0:
                ...
                if cur_pos > max_pos_days: 
                    max_pos_days = cur_pos; max_pos_dates = cur_pos_dates.copy()
            elif p < 0:
                cur_neg += 1; cur_neg_dates.append(d)
                cur_pos = 0; cur_pos_dates = []
                if cur_neg > max_neg_days: 
                    max_neg_days = cur_neg; max_neg_dates = cur_neg_dates.copy()
            else:
                cur_pos, cur_neg = 0, 0
                cur_pos_dates, cur_neg_dates = [], []

        valid_events = completed_bets[completed_bets['Event'].str.strip() != ""]
        if not valid_events.empty:
            event_profit = valid_events.groupby('Event')['Profit'].sum().reset_index()
            best_ev_row = event_profit.loc[event_profit['Profit'].idxmax()]
            worst_ev_row = event_profit.loc[event_profit['Profit'].idxmin()]
            
            best_ev_name_full = best_ev_row['Event']
            best_ev_prof = best_ev_row['Profit']
            best_ev_name = best_ev_name_full[:18] + ".." if len(best_ev_name_full) > 18 else best_ev_name_full
            
            worst_ev_name_full = worst_ev_row['Event']
            worst_ev_prof = worst_ev_row['Profit']
            worst_ev_name = worst_ev_name_full[:18] + ".." if len(worst_ev_name_full) > 18 else worst_ev_name_full
        else:
            best_ev_name_full, best_ev_name, best_ev_prof = "-", "-", 0.0
            worst_ev_name_full, worst_ev_name, worst_ev_prof = "-", "-", 0.0
        ...
        best_str = f"\n{best_ev_name} (+{best_ev_prof:.2f} €)" if best_ev_prof > 0 else "\n-"
        if c_ff3.button(f"🔥 Χρυσή Ομάδα/Παίκτης{best_str}", key="btn_ff_best", use_container_width=True):
            if best_ev_prof > 0:
                show_bets_dialog(f"🔥 Ιστορικό: {best_ev_name_full}", completed_bets[completed_bets['Event'] == best_ev_name_full], df)
            else:
                st.toast("Δεν υπάρχει ακόμα κερδοφόρα ομάδα!", icon="⚠️")

        st.markdown('<div class="marker-dark"></div>', unsafe_allow_html=True)
        worst_str = f"\n{worst_ev_name} ({worst_ev_prof:.2f} €)" if worst_ev_prof < 0 else "\n-"
        if c_ff4.button(f"🧊 Μαύρη Λίστα{worst_str}", key="btn_ff_worst", use_container_width=True):
            if worst_ev_prof < 0:
                show_bets_dialog(f"🧊 Ιστορικό: {worst_ev_name_full}", completed_bets[completed_bets['Event'] == worst_ev_name_full], df)
            else:
        ...
        with col_chart1:
            ...
            if not completed_bets.empty:
                ...
                base = alt.Chart(df_line).encode(x=alt.X('Bet_Count:Q', axis=alt.Axis(labels=False, title=None, ticks=False, grid=False)), y=alt.Y('Cumulative_Profit:Q', title="Ταμείο (€)", axis=alt.Axis(gridColor="#1f2937")))
                
                # ΟΜΑΛΗ ΚΑΜΠΥΛΗ ΜΕ 'basis'
                line = base.mark_trail(interpolate='basis').encode(color=alt.condition(alt.datum.Cumulative_Profit >= 0, alt.value('#4ade80'), alt.value('#ff4b4b')), size=alt.value(3))
                points = base.mark_circle(size=60).encode(color=alt.condition(alt.datum.Cumulative_Profit >= 0, alt.value('#4ade80'), alt.value('#ff4b4b')))
                hover_points = base.mark_circle(size=500, color="transparent").encode(tooltip=[alt.Tooltip('Ημ/νια:N', title='Ημερομηνία'), alt.Tooltip('Cumulative_Profit:Q', title='Κέρδος (€)', format='.2f')])
            ...
            with st.container(border=True):
                ...
                if edit_bet_type == "Μονό":
                    ...
                    for i in range(int(edit_num_legs)):
                        ...
                        st.markdown(f"<div style='background-color: rgba(22, 38, 59, 0.4); padding: 15px; border-radius: 12px; margin-bottom: 15px; border-left: 4px solid #4db8ff;'>", unsafe_allow_html=True)
                        c_ev, c_od, c_st = st.columns([4,2,2])
                        ed_lev_key = f"ed_pbb_ev_{i}_{selected_aa}"
                        l_ev_final = c_ev.text_input(f"Αγώνας {i+1}:", value=leg_ev, key=ed_lev_key)
                        render_suggestions(c_ev, ed_lev_key, l_ev_final, get_event_suggestions, (all_events, all_teams))
